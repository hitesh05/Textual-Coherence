...
def prepare_danish_data(danish_dataset, store_dataset_path, fact_extractor, logger=None):
    da_nlp = sp.load('da_core_news_sm')
    os.makedirs(store_dataset_path, exist_ok=True)
    logger = logger_wrapper(logger, logger_name='prepare_danish_data')
    
    for file_name in os.listdir(danish_dataset):
        if not file_name.endswith('.tsv'):
            continue
        res = []
        logger.info('processing file: %s' % file_name)
        file_path = os.path.join(danish_dataset, file_name)
        dframe =  pd.read_csv(file_path, sep='\t',  encoding='utf-8').dropna()
        for  text, domain, label in tqdm(zip( dframe['text'], dframe['domain'], dframe['rating'])):
            temp = {
                'text': text,
                'domain': domain,
                'label':label
            ...
            processed_text = re.sub(r"\s{1,}", " ", str(text))
            sents_list = [str(x.text).strip() for x in da_nlp(processed_text).sents]
            
            sents_list = [x for x in sents_list if x != '<para_break>' and x != '']
            
            temp['sentences'] = [[y.text for y in da_nlp(x)] for x in sents_list]
#            print(temp['sentences'])
            temp['facts'] = []
            # for sent in sents_list:
            #     temp['facts'].append(fact_extractor.get_triples(sent))
            res.append(temp)
        store_file_name = file_name.split('.', 1)[0]
        store_results(os.path.join(store_dataset_path, '%s.jsonl'%store_file_name), res)
#prepare_danish_data("ddisco","processed_data/",None)
def prepare_hindi_dataset(hindi_dataset, store_dataset_path, fact_extractor, logger=None):
    print("hi")
    os.makedirs(store_dataset_path, exist_ok=True)
    logger = logger_wrapper(logger, logger_name='prepare_hindi_dataset')
    
    for file_name in os.listdir(hindi_dataset):
        if not file_name.endswith('.csv'):
            continue
        res = []
        logger.info('processing file: %s' % file_name)
        file_path = os.path.join(hindi_dataset, file_name)
        dframe = pd.read_csv(file_path)
        for  text in tqdm(zip( dframe['text'])):
            temp = {
                'text': text
            }
            ...
            sents_list=sentence_tokenize.sentence_split(processed_text,"hi")
            #sents_list=re.split(r"|",processed_text)
            #print(sents_list)
            #sents_list = [x for x in sents_list if x != '<para_break>' and x != '']
            temp['sentences'] = [tokenize(x,"hi") for x in sents_list]
            temp['facts'] = []
            #print(temp['sentences'])
            list_=['_','<','>','|',"'",'।','(',')',',','▁','<']
            ans=[]
            for l in temp["sentences"]:
                ll=l.copy()
                lst1 = filter(lambda item: item not in list_,ll)
...
prepare_hindi_dataset("hindi","processed_data/",None)
