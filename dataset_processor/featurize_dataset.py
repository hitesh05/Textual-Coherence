...
class DanishFeautrizer():
    def __init__(self, args):
        if args.task not in dataset_options['danish']['tasks']:
            raise Exception('handler for %s task is not defined.' % (args.task))

        args.logger.info('featurizing the danish corpus for task: %s with model architecture: %s' % (args.task, args.arch))
        #cache the cmdline arguments
        self.args = args
    ...
    def get_task_specific_labels(self, data_input_map):
        ...
        #         labels=1 # low coherence
        #     return labels
        if self.args.task == '3-way-classification':

            # its 3-class classification problem
            labels = int(data_input_map['label']) - 1    # actual danish labels are {1=low, 2=medium, 3=high}, transformed to [0=low, 1=medium, 2=high].  
            return labels
        else:
            raise Exception('%s task specific label processing is not defined' % self.args.task)
    ...
    def load_data(self, inference):
        ...
        danish_dataset_path = os.path.join(self.args.processed_dataset_path, 'danish')
        self.args.logger.debug('<<LOADING>> danish dataset from directory: %s' % danish_dataset_path)
        for set_type in dataset_map:
            self.args.logger.debug('working on %s dataset ' % set_type)
            file_name = "%s.jsonl"%(set_type)
            input_data = load_file(os.path.join(danish_dataset_path, file_name))
            dataset_map[set_type] = input_data
            self.args.logger.debug('<Done>')
        self.args.logger.debug('--'*30)
        return dataset_map
    ...
    def featurize_dataset(self, inference=False):
        ...
        # task specific handler
        if self.args.task in ['3-way-classification']: 
            for set_type, data_list in dataset_map.items():
                for data_instance in data_list:
